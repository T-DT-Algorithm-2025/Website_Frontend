...
import type { SphereIcon, IconCloudProps } from "./icon";
...
watchEffect(() => {
  ...
  const newImageCanvases = images.map((url: any, idx: number) => {
...
function handleMouseDown(e: MouseEvent) {
  ...
  imagePositions.value.forEach((icon: SphereIcon) => {
...
onMounted(() => {
  ...
  function animate() {
    ...
    imagePositions.value.forEach((icon: any, index: number) => {
      const cosX = Math.cos(rotation.x);
      const sinX = Math.sin(rotation.x);
      const cosY = Math.cos(rotation.y);
      const sinY = Math.sin(rotation.y);
